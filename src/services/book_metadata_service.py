...
def build_book_metadata(book, container, database_service, abs_service, grimmory_client=None):
    ...
    'duration', 'isbn', 'asin', 'pages', 'hardcover_url', 'grimmory_url',
    ...
    # Grimmory metadata (description, publisher, language)
    if book.ebook_filename:
        bl_client = grimmory_client or container.grimmory_client_group()
        try:
            if bl_client and bl_client.is_configured():
                ...
                if bl_book:
                    ...
                    bl_prefix = f"GRIMMORY{'_2' if instance_id == '2' else ''}"
                    bl_base = get_service_web_url(bl_prefix) or getattr(bl_client, "base_url", "")
                    bl_url = f"{bl_base}/book/{bl_book.get('id')}?tab=view"
                    metadata["grimmory_url"] = bl_url
        except Exception as e:
            logger.debug(
                "Grimmory lookup failed for ebook_filename=%s, original=%s, client=%s: %s",
                book.ebook_filename,
                getattr(book, "original_ebook_filename", None),
                getattr(bl_client, "base_url", "?"),
                e,
            )
...
def build_service_info(book, states_by_book, container, abs_service, metadata, has_bookfusion_link):
    ...
    integrations = {
        ...
        "grimmory": bool(metadata.get("grimmory_url")),
    }
    ...
    bl_group = container.grimmory_client_group()
    services_enabled = {
        ...
        "grimmory": bl_group is not None and bl_group.is_configured(),
    }
