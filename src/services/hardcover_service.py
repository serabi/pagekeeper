...
class HardcoverService:
    ...
    def __init__(self, hardcover_client: HardcoverClient, database_service, abs_client=None):
    ...
    def push_local_rating(self, book, rating):
        ...
        try:
            ...
            if rating is not None:
                hc_rating = round(min(max(float(rating), 0), 5) * 2) / 2
    ...
    def push_initial_progress(self, book, hardcover_sync_client):
        ...
        try:
            ...
            logger.info(f"Hardcover: pushed initial progress {max_pct:.1%} for '{sanitize_log_data(book.title)}'")
        except Exception as e:
    ...
    def automatch_hardcover(self, book, hardcover_sync_client=None):
        ...
        search_strategies = [
            ...
            (lambda: self.hardcover_client.search_by_asin(asin) if asin else None, "asin", asin),
            (
                lambda: self.hardcover_client.search_by_title_author(title, author) if (title and author) else None,
                "title_author",
                title and author,
            ),
            ...
            if book_id:
                ...
                if audio_ed and audio_ed.get("audio_seconds") and audio_ed["audio_seconds"] > 0:
                    ...
                    if page_ed and page_ed.get("pages") and page_ed["pages"] > 0:
                        ...
                        match["pages"] = -1
                    logger.info(
                        f"Hardcover: '{sanitize_log_data(meta.get('title'))}' matched as audiobook ({audio_seconds}s)"
                    )
        ...
        if match:
            ...
            log_hardcover_action(
                ...
                direction="push",
                action="automatch",
                detail={"matched_by": matched_by, "hardcover_book_id": match.get("book_id"), "slug": match.get("slug")},
            )
    ...
    def set_manual_match(self, book_abs_id: str, input_str: str) -> bool:
        ...
        log_hardcover_action(
            ...
            direction="push",
            action="manual_match",
            detail={"hardcover_book_id": match["book_id"], "slug": match.get("slug"), "input": input_str},
        )
