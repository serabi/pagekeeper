...
class HardcoverSyncClient(SyncClient):
    ...
    def _handle_status_transition(self, book, hardcover_details, current_status, percentage, is_finished):
        ...
        if new_status != current_status:
            ...
            logger.info(
                f"Hardcover: '{sanitize_log_data(book.title)}' status → {status_names.get(new_status, new_status)}"
            )
    ...
    def update_progress(self, book: Book, request: UpdateProgressRequest) -> SyncResult:
        ...
        current_status = self._handle_status_transition(
        ...
        try:
            ...
            actual_pct = (
                1.0
                if is_finished and total_pages > 0
                else (min(page_num / total_pages, 1.0) if total_pages > 0 else percentage)
            )
