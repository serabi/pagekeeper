...
class CleanFlaskIntegrationTest(unittest.TestCase):
    ...
    def test_ebook_only_match(self):
        ...
        try:
            ...
            resolve_calls = self.mock_database_service.resolve_suggestion.call_args_list
            # Resolves for kosync and grimmory by ebook filename
            assert any(c == (("abcdef1234567890aabbccdd",), {"source": "kosync"}) for c in resolve_calls)
            assert any(c == (("test-ebook.epub",), {"source": "grimmory"}) for c in resolve_calls)
    ...
    def test_storyteller_only_match(self):
        ...
        # Should resolve the storyteller suggestion
        self.mock_database_service.resolve_suggestion.assert_called_once_with("st-uuid-only-456", source="storyteller")
