...
function getServiceTestPayload(service) {
  ...
  if (service === "kosync") {
    return {
      ...
      key:
        getInputValue("kosync_server_key_input") || getInputValue("KOSYNC_KEY"),
    };
  }
  return {};
}
  ...
  fetch(url)
    ...
    .then(function (data) {
      ...
      if (data.length === 0) {
        PKModal.alert({
          title: "No Libraries",
          message: "No libraries found. Check connection or try syncing first.",
        });
        return;
      }
    ...
    .catch(function (err) {
      PKModal.alert({
        title: "Error",
        message: "Failed to fetch libraries: " + err,
      });
    })
    .finally(function () {
      btn.textContent = originalText;
...
function toggleKosyncSourceMode() {
  ...
  if (!hiddenInput || !builtinSection || !externalSection || !externalUrl)
    return;
...
function copyInputValue(inputId) {
  ...
  setTimeout(function () {
    copyText.classList.remove("input-copied");
  }, 200);
}
      ...
      if (data.success) {
        ...
        PKModal.alert({
          ...
          message: parts.length
            ? parts.join(", ") + "."
            : "All reading dates are already up to date.",
        });
      } else {
        PKModal.alert({
          title: "Error",
...
function absGrimmoryOptions() {
  return {
    carry_listening_sessions:
      document.getElementById("abs_grim_sessions").checked,
    carry_bookmarks: document.getElementById("abs_grim_bookmarks").checked,
  };
}

function absGrimmoryRenderError(data) {
  PKModal.alert({
    title: "Error",
    message: "Failed: " + ((data && data.error) || "Unknown error"),
  });
}

function absGrimmoryPreview(btn) {
  var resultsEl = document.getElementById("abs_grim_results");
  btn.disabled = true;
  ...
  fetch("/api/abs-grimmory-migration/preview", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(absGrimmoryOptions()),
  })
    .then(function (r) {
      return r.json();
    })
    .then(function (data) {
      if (!data.success) {
        absGrimmoryRenderError(data);
        return;
      }
      var c = data.counts || {};
      var willMigrate = Number(c.will_migrate || 0);
      resultsEl.textContent =
        willMigrate +
        " to migrate · " +
        Number(c.already_migrated || 0) +
        " already migrated · " +
        Number(c.unmatched || 0) +
        " unmatched (no Grimmory match)";
      var migrateBtn = document.getElementById("abs_grim_migrate_btn");
      migrateBtn.disabled = willMigrate === 0;
    })
    .catch(function (err) {
      console.error("ABS→Grimmory preview error:", err);
      PKModal.alert({
        title: "Error",
        message: "An error occurred. Check console for details.",
      });
...
function absGrimmoryMigrate(btn) {
  var dryRun = document.getElementById("abs_grim_dry_run").checked;
  var doRun = function () {
    var resultsEl = document.getElementById("abs_grim_results");
    btn.disabled = true;
    btn.textContent = dryRun ? "Dry running…" : "Migrating…";
    var payload = absGrimmoryOptions();
    ...
    fetch("/api/abs-grimmory-migration/run", {
      ...
      .then(function (data) {
        ...
        var title = dryRun ? "Dry Run Complete" : "Migration Complete";
        PKModal.alert({
          title: title,
          message: parts.length
            ? parts.join(", ") + "."
      ...
      .catch(function (err) {
        console.error("ABS→Grimmory migrate error:", err);
        PKModal.alert({
          title: "Error",
          message: "An error occurred. Check console for details.",
        });
  ...
  PKModal.confirm({
    title: "Migrate to Grimmory",
    message:
      "This will mark finished books as READ in Grimmory and write reading history. Continue?",
...
async function fetchAbsLibraries() {
  ...
  try {
    ...
    var currentIds = (hiddenInput.value || "")
      .split(",")
      .map(function (s) {
        return s.trim();
      })
...
function updateAbsLibraryIds() {
  var checkboxes = document.querySelectorAll(
    '#abs_library_list input[type="checkbox"]:checked',
  );
    ...
    deviceSelect.addEventListener("change", function () {
      if (this.value === "cuda") {
        PKModal.alert({
          title: "NVIDIA GPU",
          message:
            "To use an NVIDIA GPU, you must modify your docker-compose.yml to include the 'deploy' block with 'capabilities: [gpu]' and ensure the NVIDIA Container Toolkit is installed on your host.",
        });
      }
    });
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (bfResyncBtn) {
    bfResyncBtn.addEventListener("click", function () {
      if (
        !confirm(
          "This will re-download all highlights from BookFusion. Continue?",
        )
      ) {
        return;
      }
      ...
      fetch("/api/bookfusion/sync-highlights", {
        ...
        .then(function (data) {
          if (data.success) {
            ...
            bfResyncBtn.textContent =
              "Synced (" + data.new_highlights + " new)";
