...
|-------------|-------------|----------|----------|----------|
| `not_started` | 1 | Want to Read | `LOCAL_TO_HC_STATUS['not_started']` = 1 | `HC_TO_LOCAL_STATUS[1]` = `'not_started'` |
...
| *(none)* | 6 | Ignored | TBR removal only (`HC_IGNORED`) | Not mapped (excluded from import) |
...
| `TbrItem` | `hardcover_book_id`, `hardcover_slug`, `hardcover_list_id`, `hardcover_list_name`, `source`, `book_abs_id` |

---

## 8. TBR / Want-to-Read Integration

### 8.1 Want to Read Import (Pull)

**When:** User clicks "Import Want to Read" on the TBR tab.
**Code:** `reading_bp` → `HardcoverClient.get_want_to_read_books()`

Flow:
1. Fetches all `user_books` with `status_id=1` via `me { user_books }` query
2. For each book, creates a `TbrItem` with `source='hardcover_wtr'`
3. Deduplicates by `hardcover_book_id` — skips items already in `tbr_items`
4. Auto-links to existing `Book` records via `HardcoverDetails.hardcover_book_id` match

### 8.2 Custom List Import (Pull)

**When:** User selects a list from "Import from List..." on the TBR tab.
**Code:** `reading_bp` → `HardcoverClient.get_user_lists()` + `get_list_books()`

Flow:
1. `get_user_lists()` fetches all user lists for the list picker
2. `get_list_books(list_id)` fetches books with metadata and positions
3. Creates `TbrItem` entries with `source='hardcover_list'`, `hardcover_list_id`, `hardcover_list_name`
4. Deduplicates by `hardcover_book_id`

### 8.3 Status Mapping (Bidirectional)

The `not_started` ↔ `HC_WANT_TO_READ` (1) mapping enables:
- **Push:** Books with `not_started` status push as Want to Read on Hardcover
- **Pull:** HC status_id=1 pulls as `not_started` locally
- **TBR items** that aren't in the `Book` table stay in `tbr_items` only — they don't participate in status sync

### 8.4 Auto-Linking (Push/Pull)

**Auto-link on match (pull):** When `save_hardcover_details()` stores a `hardcover_book_id`,
it checks `tbr_items` for a matching entry and sets `book_abs_id` if found.

**Push WTR on add:** When a user adds a TBR item from Hardcover search,
PageKeeper pushes `status_id=1` (Want to Read) to Hardcover via `update_status()`.
Import flows (WTR import, list import) do NOT push back — those items already exist on HC.

### 8.5 TBR Removal → Hardcover Ignored (Push)

**When:** User deletes an HC-sourced TBR item and chooses "Also remove from Hardcover".
**Code:** `tbr_bp.delete_tbr_item()` with `?remove_from_hc=true` query param.

Flow:
1. Frontend detects HC-sourced item (`source` starts with `'hardcover'`)
2. Shows a `confirm()` dialog: "Also remove from Hardcover?" (OK/Cancel)
3. If confirmed: `DELETE /api/reading/tbr/{id}?remove_from_hc=true`
4. Backend fetches the item, calls `hardcover_client.update_status(book_id, 6)` — status 6 = **Ignored**
5. Deletes the local TBR item

**Why Ignored (6)?** The Hardcover API has no `delete_user_book` mutation. Setting status to 6 (Ignored) removes the book from the Want to Read shelf and prevents re-import, since import only fetches `status_id=1`.

**If the user chooses "PageKeeper only":** `DELETE /api/reading/tbr/{id}` (no query param). The HC status is left unchanged — the book will re-import on next "Import Want to Read" if it's still `status_id=1` on HC.
